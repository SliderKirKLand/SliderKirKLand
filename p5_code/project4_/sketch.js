...
//Need to add more here -> goes with JS PDF
...
function preload() {
  ...
  dingSound = loadSound('sfx/ding.mp3');  
}
...
function setup() {
  ...
  .style('top', '50%') 
  .style('left', '50%')
  .style('transform', 'translate(-50%, -260%)') 
  .style('z-index', '2');
  ...
  globalPlayButton = createButton("")
    ...
    .style('border', '0px solid #000') // Visible border for debugging
  ...
  //result scene
  mgr.addScene(RefundScene);
  mgr.addScene(TaxFormScene);

  // Start with the Menu Scene
  mgr.showScene(TaxFormScene);
...
function renderPDF(url) {
  ...
  pdfjsLib.getDocument(url).promise.then(pdf => {
    ...
    pdf.getPage(1).then(page => {
      ...
      let viewport = page.getViewport({ scale: 1.5 }); // Adjust scale for size
...
function WelcomeScene() {
  ...
  this.enter = function () {
    ...
    // Instructions
    instructionsP = createP("Press space to start")
      .style('text-align', 'center')
...
function InputScene() {
  ...
  this.enter = function () {
    ...
    // Create a parent container with fixed 400x160px and a scrollbar
    ...
    saveButton.mousePressed(() => {
      ...
      // Save data globally
      taxData.personalInfo.firstName = firstNameInput.value();
      taxData.personalInfo.lastName = lastNameInput.value();
      taxData.personalInfo.ssn = ssnInput.value();
      taxData.personalInfo.isBlind = blindCheckbox.checked();
      taxData.personalInfo.dob = dobInput.value();
      taxData.personalInfo.phone = phoneInput.value();
      taxData.personalInfo.email = emailInput.value();
      taxData.personalInfo.isDependent = dependentCheckbox.checked();
      taxData.personalInfo.address = addressInput.value();
      taxData.personalInfo.unit = unitInput.value();
      taxData.personalInfo.city = cityInput.value();
      taxData.personalInfo.foreignAddress = foreignCheckbox.checked();
      taxData.personalInfo.state = stateInput.value();
      taxData.personalInfo.zip = zipInput.value();
      taxData.personalInfo.residencyState = residencyStateInput.value();
  ...
  this.enter = function (){
    ...
    let buttonContainer = createDiv()
      ...
      .mousePressed(() => {
        ...
        clickSound.play();
        taxData.refundInfo.routingNumber = routingNumberInput.value();
        taxData.refundInfo.accountNumber = accountNumberInput.value();
        taxData.refundInfo.accountType = checkingRadio.value() || savingsRadio.value();
  ...
  this.enter = function () {
    ...
    .style('cursor', 'pointer')
      ...
      .mousePressed(() => {
        ...
        taxData.infoQuestions.cryptoTransactions = cryptoCheckbox.checked();
        taxData.infoQuestions.foreignAccount = foreignAccountCheckbox.checked();
        taxData.infoQuestions.foreignTrust = foreignTrustCheckbox.checked();
...
function TaxFormScene() {
  ...
  this.enter = function () {
    ...
    renderPDF('image/f1040.pdf'); // Replace with the correct path to your PDF
  };
...
// Ensure renderPDF Function Works
function renderPDF(url) {
  let pdfViewer = document.getElementById('pdf-viewer');
  let pdfCanvas = document.createElement('canvas');
  let pdfContext = pdfCanvas.getContext('2d');
